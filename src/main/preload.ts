...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Sync request — main asks controller to push current state to new windows
  onSyncRequest: (cb: () => void) => {
    ipcRenderer.on('output:sync-request', () => cb())
    return () => ipcRenderer.removeAllListeners('output:sync-request')
  },
