...
export interface TimerState {
  sceneIndex: number
  elapsed: number        // seconds elapsed in current scene
  total: number          // total duration of current scene
  status: TimerStatus
}
...
const DEFAULT_SCENES: Scene[] = [
  { id: uid(), name: 'Ouverture', fullName: 'Ouverture de la conférence', duration: 300 },
  { id: uid(), name: 'Keynote', fullName: 'Keynote — présentation produit', duration: 1200 },
  { id: uid(), name: 'Table ronde', fullName: 'Table ronde — experts invités', duration: 900 },
  { id: uid(), name: 'Pause', fullName: 'Pause networking', duration: 600 },
  { id: uid(), name: 'Q&A', fullName: 'Questions & Réponses', duration: 480 },
]
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  timerStatus: 'stopped',
  elapsed: 0,
  total: DEFAULT_SCENES[0].duration,
  messages: [],
  ...
  expireMessage: (id) => set(s => ({
    messages: s.messages.map(m => m.id === id ? { ...m, active: false } : m)
  })),
  ...
  loadSessionData: (raw, filePath) => {
    try {
      ...
      set({
        scenes: data.scenes ?? DEFAULT_SCENES,
        settings: { ...DEFAULT_SETTINGS, ...(data.settings ?? {}) },
        outputSettings: { ...DEFAULT_OUTPUT, ...(data.outputSettings ?? {}) },
        currentSceneIndex: 0,
        elapsed: 0,
        total: (data.scenes?.[0]?.duration) ?? DEFAULT_SCENES[0].duration,
        timerStatus: 'stopped',
